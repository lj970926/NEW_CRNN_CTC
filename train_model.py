...
def train_model():
    ...
    with tf.variable_scope('CRNN_CTC_TRAIN', reuse=False):
    ...
    learn_rate = tf.train.exponential_decay(learning_rate, global_step, decay_steps, decay_rate)
    update_op = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
    with tf.control_dependencies(update_op):
        optimizer = tf.train.AdamOptimizer(learning_rate=learn_rate).minimize(loss=ctc_loss, global_step=global_step)
